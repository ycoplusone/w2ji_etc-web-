...
sessionStorage.setItem("badkey" , 0);
...
function getnextMelody() {
	...
	/*
	if( sessionStorage.getItem("level") ==10 || sessionStorage.getItem("level") ==20 || sessionStorage.getItem("level") ==30 ){
		window.alert( sessionStorage.getItem("level") , '단계를 성공하였습니다.');
	}
	*/
...
function checkMelody(note, key_button) {
	...
	if (cur_scale[cur_anskey[cur_anspos]] == note) { //if correct
		...
		var badkey = sessionStorage.getItem("badkey");
		badkey *= 1;
		sessionStorage.setItem("badkey" , (badkey + 1));

		if(badkey >= 3){
			//마무리
			window.alert( badkey + '번 틀리셨습니다.\n 다시 도전하세요.');			
			TIMEMANAGER.timeStop = 1; //타이머 정지 옵션
			document.getElementById("playmelodybtn").disabled = true;	//플레이 버튼 비활성화
		}
